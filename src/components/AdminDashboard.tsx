...
import { Package, Clock, CheckCircle2, Truck, Printer, X, Plus, Minus, Trash2, Ban } from "lucide-react";
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 w-full max-w-sm mx-4 overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[70vh]">
              <div ref={receiptRef} className="font-mono text-sm text-gray-800">
                ...
                {receiptOrder.processedBy && (
                  <div className="flex justify-between text-xs text-gray-500 mb-4 -mt-2">
                    <span>Processed By</span>
                    <span className="font-bold text-gray-700">{receiptOrder.processedBy}</span>
                  </div>
                )}
              ...
              >
                <div className="flex flex-col gap-4 mb-6">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {order.processedBy && (
                      <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400 flex items-center gap-1 border border-indigo-100 dark:border-indigo-800">
                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 animate-pulse"></span>
                        Admin: {order.processedBy}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {statuses.map((status) => {
                      ...
                      const isProcessedByOther = order.processedBy && order.processedBy !== currentUser.name;

                      if (status === 'Cancelled') {
                        // Cancel button is disabled once already cancelled or completed
                        const isDisabled = isCancelled || order.status === 'Completed' || Boolean(isProcessedByOther);
                        return (
                          <button
                            key={status}
                            onClick={() => !isDisabled && setConfirm({ orderId: order.id, status })}
                            disabled={isDisabled}
                            title={isProcessedByOther ? `Locked by ${order.processedBy}` : undefined}
                      ...
                      const isDisabledFlow = isPast || isCurrent || Boolean(isProcessedByOther);
                      return (
                        <button
                          key={status}
                          onClick={() => !isDisabledFlow && setConfirm({ orderId: order.id, status })}
                          disabled={isDisabledFlow}
                          title={isProcessedByOther ? `Locked by ${order.processedBy}` : (!isCancelled && isPast ? "Cannot revert to a previous status" : undefined)}
